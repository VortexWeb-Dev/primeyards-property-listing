...
<script>
    ...
    async function fetchDevelopers(page = 1) {
        ...
        const apiUrl = `${baseUrl}/crm.item.list?entityTypeId=${entityTypeId}&order[id]=desc&select[0]=id&select[1]=ufCrm6DeveloperName&start=${(page - 1) * pageSize}`;
        ...
        try {
            ...
            developerList.innerHTML = developers
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">${developer.ufCrm6DeveloperName || 'N/A'}</td>
    ...
    async function deleteDeveloper(developerId) {
        const baseUrl = 'https://primeyards.bitrix24.com/rest/8/pgu3tj84jm1lyk1z';
        const apiUrl = `${baseUrl}/crm.item.delete?entityTypeId=1036&id=${developerId}`;
